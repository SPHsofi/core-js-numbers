...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const divisible = x1 * x2 + y1 * y2;
  const divisor1 = Math.sqrt(x1 * x1 + y1 * y1);
  const divisor2 = Math.sqrt(x2 * x2 + y2 * y2);
  const res = divisible / (divisor1 * divisor2);
...
function toNumber(value, def) {
  const res = Number(value) ? value : def;
  return res;
  // throw new Error('Not implemented');
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = x1 + x2 + x3;
  const res = sum.isInteger ? sum : sum.toFixed(1);
  return res;
  // throw new Error('Not implemented');
}
